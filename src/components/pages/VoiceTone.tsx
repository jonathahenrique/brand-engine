...
export default function VoiceTone() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">Voz & Tom</h1>
      ...
      <div className="bento bento-3">
        <div className="card p-5">
          ...
          <p className="text-2xl font-bold text-[var(--text-primary)]">{voice.archetype}</p>
        </div>
        <div className="card p-5">
          <p className="section-label mb-1">Tom</p>
          <p className="text-2xl font-bold text-[var(--text-primary)]">{voice.tone}</p>
        </div>
        <div className="card p-5">
          <p className="section-label mb-1">Regras</p>
          <p className="text-2xl font-bold text-[var(--text-primary)]">{voice.rules.length}</p>
        </div>
      </div>
        ...
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {voice.personality.is.map((t) => (
              <div key={t.trait}>
                <p className="text-sm font-semibold text-[var(--text-primary)]">{t.trait}</p>
            ...
            <div key={r.rule} className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="mt-2 rounded-lg bg-[var(--bg-subtle)] px-3 py-2">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {voice.references.map((ref) => (
            <span
              key={ref}
              className="rounded-full bg-[var(--bg-muted)] px-4 py-1.5 text-xs font-medium text-[var(--text-secondary)]"
            >
              {ref}
            </span>
          ))}
